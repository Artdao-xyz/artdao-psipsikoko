...
<div class="relative mx-auto h-full">
    <!-- BACKGROUND -->
    <div class="hidden absolute right-0 top-0 z-10">
        <a on:click={ closeExperience } href={"#"}><img src="/minting/exit.png" alt="exit"></a>
    </div>
    ...
    {#if start}
        <!-- ADOPT FORM -->
        {#if question < 1}
            <a href={"#"}><img class="select-none object-contain mx-auto h-full" src="/minting/psipsikoko-minting.png" alt="Minting"></a>

            <img class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 backdrop-blur-md w-full select-none mx-auto" src="/minting/adopt-koki.png" alt="adopt koki">
        ...
        {:else}
            <img class="w-full object-cover" src="/minting/minting-background.png" alt="Q1 background">
        {/if}
            
        <!-- BLINKING -->
        {#if isVisible & question > 0}
            <img class="select-none absolute left-1/2 -translate-x-1/2 top-0 h-full w-full object-contain" src="/minting/minting-eyes.png" alt="Minting Eyes">
        {/if}
    
    {:else}
        <a on:click={startExperience} href={"#"}><img class="select-none object-contain mx-auto" src="/minting/psipsikoko-minting.png" alt="Minting"></a>
    {/if}

    <!-- QUESTION 1 -->                
    {#if question == 1}

        <svg in:fade={{ duration: fadeDuration }} out:fade={{ duration: fadeDuration }} class="absolute left-0 top-0 w-full object-cover" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 2436 1419">
    ...
    <!-- WRONG -->
    {#if wrong }
        <img in:fade={{ duration: fadeDuration }} out:fade={{ duration: fadeDuration } } class="absolute left-0 top-0 w-full object-cover" src="/minting/wrong.png" alt="Q1 background">
    {/if}

    {#if finished}
        <img class="absolute left-0 top-0 h-full w-full blur-md" src="/minting/psipsikoko-minting.png" alt="Q1 background">
